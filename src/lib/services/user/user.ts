...
export function create(user: Pick<User, 'email' | 'firstName' | 'lastName' | 'roles'>): Promise<User> {
...
export function details(userIdOrEmail: IdOrObject<User> | string): Promise<User> {
...
export function setLines(user: IdOrObject<User>, lines: IdOrObject<Line>[]): Promise<SuccessResponse> {
	const userId = extractId(user);
